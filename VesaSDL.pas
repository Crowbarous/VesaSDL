...
const
	...
	// Text styles, to combine is to OR
...
function LoadFontToPtr (name_ : ansistring; ptsize : longint; style : word) : pointer; {loading more than 1 font / custom font pointers}
procedure DestroyFont (font : pointer); {Every font loaded by LoadFontToPtr MUST be destroyed when program ends}
...
const
	// A palette for compatibility with the DOS Vesa module
...
var
	surf : pSDL_Surface;
begin
	if (currentCursor <> nil) then begin
...
begin
	...
	if (font = nil) then
		...
		writeln('Font ', name_, ' successfully loaded');
...
begin
	if (SDL_Init(SDL_INIT_VIDEO) <> 0) then
		...
		writeln('Successfully loaded SDL2');
end;
...
begin
	...
	if (render = nil) then
		...
		writeln('Successfully created sdl_render');
	...
	if (debug) then
		Writeln('Everything Done. Ready to work.');
end;
...
var
	...
	rect : tSDL_Rect;
begin
	if ((text <> '') and (font <> nil)) then begin
...
begin
	...
	for j:=0 to spritey-1 do begin
		
		for i:=0 to spritex-1 do begin
...
var
	img : pointer;
begin
	if ((lowercase(copy(filename, length(filename) - 2, 3)) = 'spr')
	or (lowercase(copy(filename, length(filename) - 2, 3)) = 'tex') ) then
...
begin
	...
	if (SDL_RenderCopy(render, tex^.image, @srcrect, @dstrect) <> 0) then begin
...
begin
	...
	if ((lowercase(copy(path, length(path) - 2, 3)) = 'spr')
	or (lowercase(copy(path, length(path) - 2, 3)) = 'tex') ) then
		img := LoadSpr(path)
	else
		img := loadImage(path);
...
var
	img : pImg;
begin
	if ((lowercase(copy(name, length(name) - 2, 3)) = 'spr')
	or (lowercase(copy(name, length(name) - 2, 3)) = 'tex') ) then
		img := LoadSpr(name)
	else
		img := loadImage(name);
...
begin
	...
	if (textwidth(text) > abs(x2 - x1)) then begin
		
		while (text <> '') do begin
			
			if (x + textwidth(text[1]) > x2) then begin
